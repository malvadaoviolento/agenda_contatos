...
Nesta versão, a Agenda de Contatos recebeu a implementação de **armazenamento dinâmico utilizando a API de Coleções do Java**.

### Principais características e conceitos

- Uso da API de Coleções do Java (`List` e `ArrayList`)
- Uso de Generics (`<String>`)
- Alocação e redimensionamento dinâmico da lista de contatos
- Uso dos métodos da API (`add`, `get`, `remove`, `size`, `indexOf`, etc.)
- Iteração simplificada com `for-each`
- Simplificação e otimização das operações de inserção, busca e remoção de registros
...
- `v0.3.0` e posteriores — alteração de contatos, modularização, introdução de classes e objetos, encapsulamento, DAO, MVC, Swing, JDBC e banco de dados.
